...
import permissionsList from "../../default-layout/permissions-list.vue";

export default {
  mixins: [modal],
  computed: {
    current_role_id: function() {
      return this.$store.getters["settings/systemPermissions/role_id"];
    },
  ...
  methods: {
    async onRoleSelected(role_id) {
      this.$store.commit("settings/systemPermissions/setRoleId", role_id);
      await this.$store.dispatch("settings/systemPermissions/loadPermissions");
    },
    close() {
      this.$store.commit("permissions/setCurrentRoleId", null);
